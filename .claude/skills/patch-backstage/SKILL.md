...
1. Remove prior **`.patch`** files and matching **`resolutions`** entries for the packages you are refreshing (search root **`package.json`**).
...
If a package resolves in multiple places at the **same** version, patch all of them.
...
2. **Delete** spurious range keys **`patch-commit`** added (e.g. **`@backstage/foo@^1.6.0`** → patch built from **`@npm:1.5.0`**), which mis-resolve other workspaces.
3. **Install:** **`yarn install`** at root;
